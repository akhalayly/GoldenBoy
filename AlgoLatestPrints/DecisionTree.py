...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        plt.savefig("Results/DecisionTree/Graph_" + file + ".png")
        plt.show()
        ...
        header = plt.table(cellText=[['']],
                           colLabels=['max_features'],
                           loc='bottom', bbox=[0, -0.125, 1.0, 0.15]
                           )
        table = ax.table(cellText=df.values, rowLabels=[2, 5, 10, 20, 30], colLabels=df.columns,
                         colWidths=[0.3, 0.3, 0.3, 0.3, 0.3], loc='bottom', cellLoc='center',rowColours=['r','r','r','r','r'],
                         colColours=['r','r','r','r','r'], bbox=[0, -0.35, 1.0, 0.3])
        table.auto_set_font_size(False)
        table.scale(1, 2)
        table._cells[(0, 0)]._text.set_text("None")
        table.set_fontsize(7)
        table.add_cell(0, -1, width=0.3, height=0.090, text="min_split")
        fig.tight_layout()
        fig.savefig("Results/DecisionTree/" + file + ".png")
        plt.show()
