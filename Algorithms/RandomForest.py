        ...
        for numFtr in [None]:
            ...
            for num in [30]:
                for train_index, test_index in splits:
                    ...
                    clf = RandomForestClassifier(criterion="gini", min_samples_split=num,
                                                 max_features=numFtr, n_estimators=100)
                    clf.fit(X_train, y_train)
                    pred_i = clf.predict(X_test)
                    results[num] += ((1 - np.mean(pred_i != y_test)) / splits.__len__())
                print(str(num) + " Accuracy is: " + str(results[num]))
