...
def roleTraitNamesFinder(Role, DatasetColumns):
    DatasetTraits = []
    for Attribute in Role:
        for DatasetColumn in DatasetColumns:
            if UpperNoSpace(Attribute).lower() in DatasetColumn.lower() and DatasetColumn not in DatasetTraits:
...
def roleTraitIndexesFinder(Role, DatasetColumns):
    wantedIndexes = []
    for Attribute in Role:
        for DatasetColumnIndex in range(len(DatasetColumns)):
            if UpperNoSpace(Attribute).lower() in DatasetColumns[DatasetColumnIndex].lower() and \
...
def scoreByRole(playerRow, traitsIndexes):
    ...
    for traitsIndex in traitsIndexes:
        if playerRow[traitsIndex] != "nan" and playerRow[traitsIndex] != "" and not posT.DatasetColumns[traitsIndex].startswith("Height"):
...
def findBestPosition(playerRow):
    ...
    for role, roleName in zip(posT.allRoles_Traits, posT.allRoles):
        allScores.append((roleName, scoreByRole(playerRow, roleTraitIndexesFinder(role, posT.DatasetColumns))))
